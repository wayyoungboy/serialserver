...
This manual is for VSP users and field operators. VSP uses a field-side serial agent, a cloud relay, and a desktop-side local TCP gateway. Desktop tools connect to a local endpoint such as `127.0.0.1:7000`, and VSP forwards bytes to the field serial device.
...
| Field operator | `device-agent` | DeviceKey | Open the physical serial port and publish a mapping |
| Desktop user | VSPManager or `desktop-gateway` | Username and password, or JWT after login | Select an online mapping and start a local TCP gateway |

Important boundaries:

- DeviceKey is only for the field-side `device-agent`.
...
Run `device-agent` on the machine connected to the physical serial port:
...
- `device-agent` logs `device registered`.
...
The test uses a Linux pseudo-terminal, starts the real server and both clients, and verifies TCP-to-serial and serial-to-TCP echo.

## 9. Troubleshooting

### Mapping List Is Empty

- Check that `device-agent` is running.
